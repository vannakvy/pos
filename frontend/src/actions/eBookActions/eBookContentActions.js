import {    
    ...
    UPDATE_CONTENT_REQUEST,} from '../../constants/eBookConstants/eBookContentConstants'
...
const addContent = (title,section, lang)=>async(dispatch)=>{
   ...
   const {data} =await axios.post(`/api/ebook/contents`,{title, section,lang});
...
const updateContent = (id,title,section)=>async(dispatch)=>{
   ...
   const {data} =await axios.put(`/api/ebook/contents/${id}`,{title,section});
