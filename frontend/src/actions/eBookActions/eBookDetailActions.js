...
const addDetail = (title,contents,id)=>async(dispatch)=>{
    try{
        ...
        const {data} = await axios.post(`/api/ebook/details`,{title,contents,id})
        dispatch({
...
const getOneDetail = (id)=>async(dispatch)=>{
    try{
        ...
        dispatch({type: GET_ONE_DETAIL_FAIL,
        payload: error.message})
    }
}
...
const updateDetail = (title,contents,id)=>async(dispatch)=>{
   ...
   const {data} =await axios.put(`/api/ebook/details/${id}`,{title,contents});
